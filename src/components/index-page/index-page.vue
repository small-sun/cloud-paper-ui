        ...
        <!-- <div class="roomlist__elem--password">
            <div id="popup" class='modal' role="dialog" aria-labelledby="Modal title" aria-describedby="Modal description">
                <div class="modal-content">
                    ...
                    <input type="text" autofocus v-model:value="inputToken">
                    <button type="button" @click.stop="enterRoom">
                      进入房间<router-link :to="url"></router-link>
                    </button>
                </div>
            </div>
        </div> -->
...
<script type="text/ecmascript-6">
 ...
 export default {
   ...
   methods: {
     ...
     createRoom () {
       let url = 'http://10.19.221.23:4000/token/create';
       axios.get(url).then((res, req) => {
         res = res.data;
         if (res.errcode === 0) {
           this.changeId('owner');
